...
export async function recordLead({
  ...
  const notesLines = [
    ...
    leadRecord.realtorAgent ? `[Referring Realtor/Brokerage]: ${leadRecord.realtorAgent}` : '',
  ...
  // 3. Forward to Google Apps Script Webhook (Google Sheets + Zero-Cost Reliable Email Gateway)
  const appsScriptUrl = process.env.APPS_SCRIPT_WEBHOOK_URL || 'https://script.google.com/macros/s/AKfycbyfxYncwB7xTqMJJHe1v5_70VNfV9BjNkH__92Ni2v5RMn83JrDjHRqpM6mT5Bs0x62lg/exec';
    ...
    try {
      const response = await fetch(appsScriptUrl, {
        ...
        body: JSON.stringify({
          ...
          realtorAgent: leadRecord.realtorAgent,
          preferredDate: leadRecord.preferredDate,
          sqft: leadRecord.sqft,
          propertyType: leadRecord.propertyType,
          serviceType: leadRecord.serviceType,
          foundation: leadRecord.foundation,
          ageTier: leadRecord.ageTier,
          addons: formattedAddons,
          estimatedTotal: leadRecord.estimatedTotal,
      ...
      if (response.ok) {
        console.log('[LEADS ENGINE] Successfully forwarded to Google Sheets and dual email dispatch via Apps Script.');
        webhookSuccess = true;
      } else {
  ...
  // 4. Send Email Notification directly (Fallback if SMTP credentials provided)
  ...
  if (emailPass) {
    try {
          ...
          <div style="font-family: Arial, sans-serif; padding: 24px; border: 2px solid #b91c1c; border-radius: 8px; background: #ffffff; max-width: 650px;">
            ...
            <div style="background: #fef2f2; border-left: 4px solid #ef4444; padding: 12px; margin-top: 16px; border-radius: 4px;">
              <p style="margin: 0; font-size: 0.85rem; color: #991b1b; line-height: 1.4;">
                <strong>Action Required:</strong> Please contact the client within 20 minutes to confirm inspector arrival time, access, and schedule any requested auxiliary partners (Pool, Termite, Radon, Sewer Scope). Sunday is strictly by appointment only.
              </p>
            </div>
          </div>
        `
      ...
      console.log('[LEADS ENGINE] Email notification sent successfully to inspect@foresightcmi.com and plsinspectnow@gmail.com!');
      emailSent = true;
    } catch (mailErr) {
      console.error('[LEADS ENGINE] Direct SMTP email send skipped/failed (using Apps Script gateway):', mailErr.message);
    }
  }
